...
export default function SignupPage() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    setError(null);
    try {
      const callbackUrl = plan ? `/dashboard?upgrade=${plan}` : '/dashboard';
        ...
        <div className="border border-[#424242] bg-[#111820] p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-[#9BA4B0]">
              {plan
                ? `Sign up to get started with ${plan.charAt(0).toUpperCase() + plan.slice(1)}`
                : 'From red to ready.'
              }
            </p>
          </div>
          ...
          {plan && (
            <div className="mb-6 p-4 border border-[#22C55E]/30 bg-[#22C55E]/10">
              <div className="flex items-center justify-between">
                <span className="text-[#22C55E] font-medium">
                  {plan === 'premium' ? 'Premium Plan' : 'Advisor Plan'}
                </span>
                <span className="text-white font-bold">
                  {plan === 'premium' ? '$9.99/mo' : '$14.99/mo'}
                </span>
              </div>
              <p className="text-xs text-[#9BA4B0] mt-1">
                You&apos;ll complete payment after signing up
              </p>
            </div>
          )}
          ...
          <div className="mt-6 pt-6 border-t border-[#424242]">
            ...
            <p className="text-xs text-[#6B7280] text-center mt-2">
              No account needed. See how it works with sample data.
            </p>
          </div>

          <p className="text-xs text-[#6B7280] text-center mt-6">
            By signing up, you agree to our{' '}
            <Link href="/terms" className="text-[#9BA4B0] hover:underline">
              Terms of Service
            </Link>{' '}
            and{' '}
            <Link href="/privacy" className="text-[#9BA4B0] hover:underline">
              Privacy Policy
            </Link>
          </p>
        </div>
